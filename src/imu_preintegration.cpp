...
: node_(node_), params_(params)
{
	...
	imu2Lidar = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(-params_->extTrans.x(), -params_->extTrans.y(), -params_->extTrans.z()));
  lidar2Imu = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(params_->extTrans.x(), params_->extTrans.y(), params_->extTrans.z()));
	...
	pubImuOdometry = node_->create_publisher<nav_msgs::msg::Odometry>(params_->odom_topic+"_incremental", qos_imu);

	boost::shared_ptr<gtsam::PreintegrationParams> p = gtsam::PreintegrationParams::MakeSharedU(params_->imu_gravity);
	p->accelerometerCovariance  = gtsam::Matrix33::Identity(3,3) * pow(params_->imu_acc_noise, 2); // acc white noise in continuous
	p->gyroscopeCovariance      = gtsam::Matrix33::Identity(3,3) * pow(params_->imu_gyro_noise, 2); // gyro white noise in continuous
	...
	noiseModelBetweenBias = (gtsam::Vector(6) << params_->imu_acc_bias_density, params_->imu_acc_bias_density, params_->imu_acc_bias_density, params_->imu_gyro_bias_density, params_->imu_gyro_bias_density, params_->imu_gyro_bias_density).finished();
...
bool IMUPreintegration::failureDetection(const gtsam::Vector3& velCur, const gtsam::imuBias::ConstantBias& biasCur)
{
	...
	if (vel.norm() > 30)
	{
		RCLCPP_WARN(node_->get_logger(), "Large velocity, reset IMU-preintegration!");
		return true;
	}
	...
	{
		RCLCPP_WARN(node_->get_logger(), "Large bias, reset IMU-preintegration!");
		return true;
	}
...
void IMUPreintegration::imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
{
    ...
    imuConverter(*imu_raw, params_->extRot, params_->extQRPY);
	...
	odometry.header.frame_id = params_->odometry_frame;
...
// 	auto ImuP = std::make_shared<lio_sam::IMUPreintegration>(options);
// 	e.add_node(ImuP);
